...
namespace ORB_SLAM3 {
    ...
    public:
        ...
        bool ReconstructWithTwoViewswithLine(const Frame& InitialFrame, const Frame& CurrentFrame, const std::vector<int> &vMatches12,
                                             cv::Mat &R21, cv::Mat &t21, std::vector<cv::Point3f> &vP3D, std::vector<bool> &vbTriangulated,
                                             vector<pair<int,int>> &vLineMatches,vector<cv::Point3f> &vLineS3D, vector<cv::Point3f> &vLineE3D,vector<bool> &vbLineTriangulated);
