...
public:
    ...
    // Constructor for Monocular cameras with lines
    Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor, Lineextractor* Lineextractor, ORBVocabulary* voc, LineVocabulary* voc_l, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
