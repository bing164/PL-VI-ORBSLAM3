...
void EdgeLineProjectXYZ::linearizeOplus() {
    float fx = pCamera->getParameter(0);
    float fy = pCamera->getParameter(1);
    float cx = pCamera->getParameter(2);
    float cy = pCamera->getParameter(3);
//    cout << "K = " << fx << " " << fy << " " << cx << " " << cy << " " << endl;

    double lx = _measurement(0);
    double ly = _measurement(1);
//    cout << "line2 = " << lx << "  " << ly << endl;

    const VertexPose* vj = static_cast<const VertexPose*>(_vertices[1]);
    const g2o::VertexSBAPointXYZ* vi = static_cast<g2o::VertexSBAPointXYZ*>(_vertices[0]);

    const Eigen::Matrix3d &Rcw = vj->estimate().Rcw[0];
    const Eigen::Vector3d &tcw = vj->estimate().tcw[0];
    const Eigen::Vector3d Xc = Rcw * vi->estimate() + tcw;
    const Eigen::Vector3d Xb = vj->estimate().Rbc[0]*Xc+vj->estimate().tbc[0];
    const Eigen::Matrix3d &Rcb = vj->estimate().Rcb[0];
    ...
    _jacobianOplusXi = 1. * invz * tmp * Rcw;
...
//    tmp(0,0) = fx * lx;
//    tmp(0,1) = fy * ly;
//    tmp(0,2) = -(fx * lx * Xc(0) + fy * ly * Xc(1)) * (1 / Xc(2));
//
////    _jacobianOplusXj = 1.0 * (1 / Xc(2)) * tmp * Rcb * SE3deriv;  // 优化变换矩阵,对应vj
//    _jacobianOplusXi = 1.0 * (1 / Xc(2)) * tmp * Rcw;   // 优化空间点对应vi
}
