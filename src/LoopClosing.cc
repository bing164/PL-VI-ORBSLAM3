...
//    cout << "MergeMap init ID: " << pMergeMap->GetInitKFid() << "       CurrMap init ID: " << pCurrentMap->GetInitKFid() << endl;
    ...
    cout << "updating current map" << endl;
    {
        ...
        vector<MapLine*> vpMergeMapMLs = pMergeMap->GetAllMapLines();
        cout << "Merge numbers = " << vpMergeMapMPs.size() << " points,  " << vpMergeMapMLs.size() << " lines" << endl;
        ...
        for (MapLine* pMLi : vpMergeMapMLs) {
...
//    cout << "Update relationship between KFs" << endl;
    ...
    set<MapPoint*> spMapPointMerge;
    set<MapLine*> spMapLineMerge;
    for(KeyFrame* pKFi : mvpMergeConnectedKFs)
    {
        set<MapPoint*> vpMPs = pKFi->GetMapPoints();
        set<MapLine*> vpMLs = pKFi->GetMapLines();
        spMapPointMerge.insert(vpMPs.begin(),vpMPs.end());
    ...
    cout << "spMapLineMerge.size() " << spMapLineMerge.size() << endl;
...
//    SearchAndFuse(vpCurrentConnectedKFs, vpCheckFuseMapPoint);
    SearchAndFuseWithLine(vpCurrentConnectedKFs,vpCheckFuseMapPoint,vpCheckFuseMapLine);
...
//    Optimizer::MergeInertialBA(pCurrKF, mpMergeMatchedKF, &bStopFlag, pCurrentMap,CorrectedSim3);
    Optimizer::MergeInertialBAWithLine(pCurrKF, mpMergeMatchedKF, &bStopFlag, pCurrentMap,CorrectedSim3);
...
void LoopClosing::SearchAndFuseWithLine(const vector<KeyFrame *> &vConectedKFs, vector<MapPoint *> &vpMapPoints,
                                        vector<MapLine *> &vpMapLines) {
    ORBmatcher matcher(0.8);
    LineMatcher lineMatcher;

    int total_replaces1 = 0;
    cout << "FUSE-POSE: Initially there are " << vpMapPoints.size() << " MPs" << endl;
    cout << "FUSE-POSE: Intially there are " << vConectedKFs.size() << " KFs" << endl;
    cout << "FUSE-POSE: Intially there are " << vpMapLines.size() << " MLs" << endl;
    for(auto mit=vConectedKFs.begin(), mend=vConectedKFs.end(); mit!=mend;mit++)
    {
        ...
        total_replaces1 += num_replaces;

        vector<MapLine*> vpReplaceLine(vpMapLines.size(),static_cast<MapLine*>(NULL));
        lineMatcher.Fuse(pKF,vpMapLines);
//        cout << "FUSE-POSE: KF " << pKF->mnId << " ->" << num_replaces << " MLs fused" << endl;
    }
    cout << "FUSE-POSE: " << total_replaces1 << " MPs had been fused" << endl;
}
...
//        Optimizer::FullInertialBAwithLine(pActiveMap,7,false,nLoopKF,&mbStopGBA);
