...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mvuRight(F.mvuRight), mvDepth(F.mvDepth), mvDepth_l(F.mvDepth_l), mDescriptors(F.mDescriptors.clone()),mvKeyLineFunctions(F.mvKeyLineFunctions),
...
vector<size_t> KeyFrame::GetLinesInArea(const float &x1, const float &y1, const float &x2, const float &y2,
                                        const float &r, const int minLevel, const int maxLevel) const {
    vector<size_t> vIndices;
    vector<KeyLine> vkl = this->mvKeys_Line;
