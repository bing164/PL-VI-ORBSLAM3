...
void Lineextractor::operator()( const cv::Mat& img, const cv::Mat& mask, 
            std::vector<cv::line_descriptor::KeyLine>& keylines, cv::Mat& descriptors_line, vector<Eigen::Vector3d> &keylineFunction)
{
...
//            cout << "lsd->mvScaleFactor[i] = " << lsd->mvScaleFactor[i] << endl;
...
//                keylines[k].octave = i;
        ...
        for(vector<KeyLine>::iterator it=keylines.begin(); it!=keylines.end(); ++it) {
            ...
            keylineFunction.push_back(lineF);
