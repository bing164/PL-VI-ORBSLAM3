...
{
    cout << "start GlobalBundleAdjustemnt" << endl;
    vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
    vector<MapPoint*> vpMP = pMap->GetAllMapPoints();
    BundleAdjustment(vpKFs,vpMP,nIterations,pbStopFlag, nLoopKF, bRobust);
    cout << "end GlobalBundleAdjustemnt" << endl;
}
...
{
    cout << "start GlobalBundleAdjustemntWithLines" << endl;
    vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
    vector<MapPoint*> vpMP = pMap->GetAllMapPoints();
    vector<MapLine*> vpML = pMap->GetAllMapLines();
//    BundleAdjustmentWithLines(vpKFs,vpMP,vpML,nIterations,pbStopFlag, nLoopKF, bRobust);
    BundleAdjustmentWithLine2(vpKFs,vpMP,vpML,nIterations,pbStopFlag, nLoopKF, bRobust);
    cout << "end GlobalBundleAdjustemntWithLines" << endl;
}
...
void Optimizer::BundleAdjustmentWithLine2(const std::vector<KeyFrame *> &vpKFs, const std::vector<MapPoint *> &vpMP,
                                          const std::vector<MapLine *> &vpML, int nItertions, bool *pbStopFlag,
...
//    int totalEdges = 0;
    ...
    for (size_t i = 0; i < vpML.size(); i++) {
        ...
        for (map<KeyFrame*, size_t>::const_iterator mit = observations.begin(); mit != observations.end(); mit++) {
            ...
            EdgeLineProjectXYZ2 *es = new EdgeLineProjectXYZ2();
            ...
            es->pCamera = pKF->mpCamera;
            optimizer.addEdge(es);

            EdgeLineProjectXYZ2 *ee = new EdgeLineProjectXYZ2();
...
//        cout << "GBA: Mono points: " << PEMono / PNMono << " ";
    ...
    for (size_t i = 0; i < vpML.size(); i++) {
        ...
        if(nLoopKF==pMap->GetOriginKF()->mnId) {
            ...
            pML->SetWorldPos(vStartPoint->estimate(),vEndPoint->estimate());
            pML->UpdateNormalAndDepth();
        } else {
            pML->mPosGBA << vStartPoint->estimate(), vEndPoint->estimate();
...
void Optimizer::FullInertialBAwithLine(Map *pMap, int its, const bool bFixLocal, const unsigned long nLoopId,
                                       bool *pbStopFlag, bool bInit, float priorG, float priorA,
                                       Eigen::VectorXd *vSingVal, bool *bHess) {
    cout << "start FullInertialBAwithLine.............." << endl;
    ...
    long unsigned int maxMapPointId = maxKFid * 5;
    ...
    int maxMapLineId = maxMapPointId;
    vector<bool> vbNotIncludedMLine(vpMLs.size(),false);
    ...
    for (size_t i = 0; i < vpMLs.size(); i++) {
        ...
        const std::map<KeyFrame*, size_t> observation = pML->GetObservations();
        bool bAllFixed = true;

        for (map<KeyFrame*, size_t>::const_iterator mit = observation.begin(); mit != observation.end(); mit++) {
            KeyFrame* pKF = mit->first;
            ...
            Eigen::Vector3d lineObs = pKF->mvKeyLineFunctions[mit->second];
//            cout << "line1 = " << lineObs.transpose() << endl;
...
//            optimizer.removeVertex(vStartPoint);
//            optimizer.removeVertex(vEndPoint);
//            vbNotIncludedMLine[i] = true;
//        } else {
//            vbNotIncludedMLine[i] = false;
//        }
...
//            linePos << vStartPoint->estimate(), vEndPoint->estimate();
    ...
    cout << "end FullInertialBAwithLine.............." << endl;
...
{
    cout << "start FullInertialBA" << endl;
    long unsigned int maxKFid = pMap->GetMaxKFid();
    const vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
    const vector<MapPoint*> vpMPs = pMap->GetAllMapPoints();
    const vector<MapLine*> vpMLs = pMap->GetAllMapLines();
...
//        }
    ...
    const unsigned long iniMLSid = maxKFid*5 + vpMPs.size();
...
//    cout << "start PoseOptimizationWithLine " << endl;
        g2o::SparseOptimizer optimizer;
        g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
...
//        Sophus::SE3<float> Tcw = pFrame->GetPose();
        ...
        // Set MapLine vertices
        const int NL = pFrame->N_l;
        vector<ORB_SLAM3::EdgeLine2ProjectXYZOnlyPose *> vpEdgesLineSp;
        vector<size_t> vnIndexLineEdgeSp;
        vpEdgesLineSp.reserve(NL);
        vnIndexLineEdgeSp.reserve(NL);

        vector<ORB_SLAM3::EdgeLine2ProjectXYZOnlyPose *> vpEdgesLineEp;
        ...
        // Set MapLine vertices
        {
            ...
            for (int i = 0; i < NL; i++) {
                ...
                if (pML) {
                    ...
                    ORB_SLAM3::EdgeLine2ProjectXYZOnlyPose *els = new ORB_SLAM3::EdgeLine2ProjectXYZOnlyPose();
                    ...
                    els->pCamera = pFrame->mpCamera;
                    els->Xw = pML->GetWorldPos().head(3);

                    optimizer.addEdge(els);
                    vpEdgesLineSp.push_back(els);
                    vnIndexLineEdgeSp.push_back(i);

                    ORB_SLAM3::EdgeLine2ProjectXYZOnlyPose *ele = new ORB_SLAM3::EdgeLine2ProjectXYZOnlyPose();
                    ...
                    ele->pCamera = pFrame->mpCamera;
                    ele->Xw = pML->GetWorldPos().tail(3);
        ...
        for (size_t it = 0; it < 4; it++) {
            ...
            for (size_t i = 0, iend = vpEdgesLineSp.size(); i < iend; i++) {
                ORB_SLAM3::EdgeLine2ProjectXYZOnlyPose *e1 = vpEdgesLineSp[i]; //线段起始点
                ORB_SLAM3::EdgeLine2ProjectXYZOnlyPose *e2 = vpEdgesLineEp[i]; //线段终止点
...
//        cout << "end PoseOptimizationWithLine " << endl;
        return nInitialCorrespondences-nBad;
    }
...
void Optimizer::LocalBundleAdjustmentwithLine(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF) {
    list<KeyFrame*> lLocalKeyFrames;
...
//    unsigned long maxPointId = maxKFid*5;
...
//            continue;
//
//        g2o::VertexSBAPointXYZ *vStartPoint = n
//    }
    ...
    void Optimizer::MergeInertialBAWithLine(KeyFrame* pCurrKF, KeyFrame* pMergeKF, bool *pbStopFlag, Map *pMap, LoopClosing::KeyFrameAndPose &corrPoses)
    {
        const int Nd = 6;
        const unsigned long maxKFid = pCurrKF->mnId;
        ...
        for (list<MapLine*>::iterator lit = lLocalMapLines.begin(), lend = lLocalMapLines.end(); lit != lend; lit++) {
            ...
            g2o::VertexSBAPointXYZ *vStartPoint = new g2o::VertexSBAPointXYZ();
            vStartPoint->setEstimate(pML->GetWorldPos().head(3));
            const int id1 = (2 * pML->mnId) + 1 + maxPointId;
            vStartPoint->setId(id1);
            vStartPoint->setMarginalized(true);
            optimizer.addVertex(vStartPoint);
            ...
            g2o::VertexSBAPointXYZ *vEndPoint = new g2o::VertexSBAPointXYZ();
            vEndPoint->setEstimate(pML->GetWorldPos().tail(3));
            const int id2 = (2 * (pML->mnId + 1)) + maxPointId;
            vEndPoint->setId(id2);
            vEndPoint->setMarginalized(true);
            optimizer.addVertex(vEndPoint);
            ...
            for (map<KeyFrame*,size_t>::const_iterator mit = observations.begin(); mit != observations.end(); mit++) {
                ...
                if (pKF->isBad())
                    continue;
                if ((pKF->mnBALocalForKF!=pCurrKF->mnId) && (pKF->mnBAFixedForKF!=pCurrKF->mnId))
                    continue;
                if (pKF->mnId>maxKFid){
                    Verbose::PrintMess("ID greater than current KF is", Verbose::VERBOSITY_NORMAL);
                    continue;
                }
                if(optimizer.vertex(id1)==NULL || optimizer.vertex(id2)==NULL || optimizer.vertex(pKF->mnId)==NULL)
                    continue;
        ...
        for(list<MapLine*>::iterator lit=lLocalMapLines.begin(), lend=lLocalMapLines.end(); lit!=lend; lit++) {
            MapLine* pML = *lit;
            g2o::VertexSBAPointXYZ *vStartPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex((2 * pML->mnId) + 1 + maxPointId));
            g2o::VertexSBAPointXYZ *vEndPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(2 * (pML->mnId + 1) + maxPointId));
            pML->SetWorldPos(vStartPoint->estimate(),vEndPoint->estimate());
            pML->UpdateNormalAndDepth();
        }
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections)
{
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        cv::Mat P3Dw = pMP->GetWorldPos();
//        cout << "point's pose11111 = " << P3Dw.t() << endl;
        Eigen::Matrix<double,3,1> eigP3Dw = Converter::toVector3d(P3Dw);
        Eigen::Matrix<double,3,1> eigCorrectedP3Dw = correctedSwr.map(Srw.map(eigP3Dw));

        cv::Mat cvCorrectedP3Dw = Converter::toCvMat(eigCorrectedP3Dw);
//        cout << "point's pose22222 = " << cvCorrectedP3Dw.t() << endl;
        pMP->SetWorldPos(cvCorrectedP3Dw);
//        cout << "point's pose33333 = " << pMP->GetWorldPos().t() << endl;
...
//    cout << "********************************" << endl << endl;
    // Correct lines. Transform to "non-optimized" reference keyframe pose and transform back with optimized pose
    cout << "Correct lines = " << vpMLs.size() << endl;
    for(size_t i=0, iend=vpMLs.size(); i<iend; i++)
    {
        MapLine* pML = vpMLs[i];
        ...
        Vector6d P3Dw = pML->GetWorldPos();
//        cout << "line's pose11111 = " << P3Dw.transpose() << endl;
        Eigen::Matrix<double,3,1> eigP3Dw_sp = P3Dw.head(3);
        Eigen::Matrix<double,3,1> eigP3Dw_ep = P3Dw.tail(3);
        Eigen::Matrix<double,3,1> eigCorrectedP3Dw_sp = correctedSwr.map(Srw.map(eigP3Dw_sp));
        Eigen::Matrix<double,3,1> eigCorrectedP3Dw_ep = correctedSwr.map(Srw.map(eigP3Dw_ep));
        ...
        //cv::Mat cvCorrectedP3Dw_ep = Converter::toCvMat(eigCorrectedP3Dw_ep);
        pML->SetWorldPos(eigCorrectedP3Dw_sp, eigCorrectedP3Dw_ep);
//        cout << "line's pose22222 = " << eigCorrectedP3Dw_sp.transpose() << " " << eigCorrectedP3Dw_ep.transpose() << endl;
//        cout << "line's pose33333 = " << pML->GetWorldPos().transpose() << endl;
        pML->UpdateNormalAndDepth();
    }
    pMap->IncreaseChangeIndex();
}
