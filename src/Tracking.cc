...
//            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpLineextractorLeft,mpORBVocabulary,mpLineVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);

        }
        else
            mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft,mpLineextractorLeft,mpORBVocabulary,mpLineVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
    }
...
//        StereoInitializationWithLines();
        MonocularInitializationWithLines();
...
void Tracking::MonocularInitializationWithLines() {
    if(!mpInitializer)
    {
        // Set Reference Frame
        if(mCurrentFrame.mvKeys.size()>100)
...
//        cout << "the number line match = " << nlineMatches << endl;
        ...
        cv::Mat tcw; // Current Camera Translation
        vector<bool> vbTriangulated; // Triangulated Correspondences (mvIniMatches)
        vector<bool> vbLineTriangulated;  //匹配的线特征是否能够三角化

//        if(mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,Rcw,tcw,mvIniP3D,vbTriangulated))
        if(mpCamera->ReconstructWithTwoViewswithLine(mInitialFrame,mCurrentFrame,mvIniMatches,Rcw,tcw,mvIniP3D,vbTriangulated,mvLineMatches,mvLineS3D,mvLineE3D,vbLineTriangulated))
        {
//            cout << "1 = " << vbTriangulated.size() << " 2 = " << vbLineTriangulated.size() << endl;
            for(size_t i=0, iend=mvIniMatches.size(); i<iend;i++)
            {
                if(mvIniMatches[i]>=0 && !vbTriangulated[i])
                {
            ...
            for(size_t i = 0, iend = mvLineMatches.size(); i < iend; i++) {
                if(mvLineMatches[i].second >= 0 && !vbLineTriangulated[i]) {
...
//            cout << "1 = " << nmatches << " 2 = " << nlineMatches << endl;
            // Set Frame Poses
            mInitialFrame.SetPose(cv::Mat::eye(4,4,CV_32F));
            cv::Mat Tcw = cv::Mat::eye(4,4,CV_32F);
            Rcw.copyTo(Tcw.rowRange(0,3).colRange(0,3));
...
{
//    cout << "start CreateInitialMapMonocular " << endl;
    // Create KeyFrames
...
//        cout << "pMP = " << worldPos.t() << endl;
    ...
    for (size_t i = 0; i < mvLineMatches.size(); i++) {
        ...
        // creat mapline
        Eigen::Vector3d sp3D, ep3D;
        sp3D << mvLineS3D[i].x, mvLineS3D[i].y, mvLineS3D[i].z;
        ep3D << mvLineE3D[i].x, mvLineE3D[i].y, mvLineE3D[i].z;
//        cout << "start = " << sp3D.transpose() << " end = " << ep3D.transpose() << endl;

        MapLine* pML = new MapLine(sp3D,ep3D,pKFcur,mpAtlas->GetCurrentMap());
...
//    Optimizer::GlobalBundleAdjustemntWithLines(mpAtlas->GetCurrentMap(),20);
    ...
    for (size_t iML = 0; iML < vpAllMapLines.size(); iML++) {
        if(vpAllMapLines[iML]) {
            ...
            pML->SetWorldPos(pML->GetWorldPos().head(3) * invMedianDepth, pML->GetWorldPos().tail(3) * invMedianDepth);
        }
    }
...
//    mnLastRelocFrameId = mInitialFrame.mnId;
    ...
    initID = pKFcur->mnId;
//    cout << "end CreateInitialMapMonocular " << endl;
...
{
//    cout << "start TrackReferenceKeyFrameWithLines " << endl;
    // Compute Bag of Words vector
    mCurrentFrame.ComputeBoW();
...
//    cout << "mCurrentFrame.n_inliers = " << mCurrentFrame.n_inliers << endl;
...
//    cout << "end TrackReferenceKeyFrameWithLines " << endl;
    // TODO check these conditions
    if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO)
        return true;
    else
...
{
//    cout << "start TrackWithMotionModelWithLines with lines" << endl;
    ORBmatcher matcher(0.9,true);
...
//    cout << "end TrackWithMotionModelWithLines with lines" << endl;
    if(mbOnlyTracking)
    {
...
{
//    cout << "start TrackLocalMapWithLines" << endl;
    // We have an estimation of the camera pose and some map points tracked in the frame.
    // We retrieve the local map and try to find matches to points in the local map.
    mTrackedFr++;
...
//    cout << "mnMatchesInliers_l = " << mnMatchesInliers_l << endl;
    mnMatchesInliers = mnMatchesInliers + mnMatchesInliers_l;
//    cout << "end TrackLocalMapWithLines" << endl;
...
{
//    cout << "start NeedNewKeyFrameWithLines" << endl;
    if(((mSensor == System::IMU_MONOCULAR) || (mSensor == System::IMU_STEREO)) && !mpAtlas->GetCurrentMap()->isImuInitialized())
    {
        if (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.25)
            return true;
...
//    cout << "end NeedNewKeyFrameWithLines" << endl;
    if(((c1a||c1b||c1c) && c2)||c3 ||c4)
    {
        // If the mapping accepts keyframes, insert keyframe.
        // Otherwise send a signal to interrupt BA
...
{
//    cout << "start CreateNewKeyFrameWithLines" << endl;
    if(mpLocalMapper->IsInitializing())
        return;
...
//    cout << "lmatches = " << matches_12.size() << endl;
    ...
    for (int i1 = 0, nmatches_12 = matches_12.size(); i1 < nmatches_12; ++i1) {     // remove maplines that do not match with orientation and position in image
        ...
        if(true) {
            // check for orientation
            double theta1 =  atan2((mCurrentFrame.mvKeysUn_Line[i2].endPointY - mCurrentFrame.mvKeysUn_Line[i2].startPointY),
            (mCurrentFrame.mvKeysUn_Line[i2].endPointX - mCurrentFrame.mvKeysUn_Line[i2].startPointX));
            double theta2 = pML->mnTrackangle;
//            cout << "theta1 = " << theta1 << " theta2 = " << theta2 << endl;
//            if (theta1 > M_PI) theta1 =
...
//    cout << "mCurrentFrame.n_inliers_ls = " << mCurrentFrame.n_inliers_ls << endl;
...
void Tracking::ReconstructLine(vector<Match> &vLineMatches, cv::Mat &K, cv::Mat &R21, cv::Mat &t21,
    ...
    for (size_t i = 0, iend = vLineMatches.size(); i < iend; i++) {
        const KeyLine &kl1 = mCurrentFrame.mvKeys_Line[vLineMatches[i].first];
        const KeyLine &kl2 = mCurrentFrame.mvKeys_Line[vLineMatches[i].second];

        Vector3d sp_l1; sp_l1 << kl1.startPointX, kl1.startPointY, 1.0;
        Vector3d ep_l1; ep_l1 << kl1.endPointX, kl1.endPointY, 1.0;

        Vector3d lineF1, lineF2;     //直线方程
        lineF1 << sp_l1.cross(ep_l1);
//        cout << "-------------------" << endl;
//        cout<<lineF1(0)<<","<<lineF1(1)<<","<<lineF1(2)<<endl;
        lineF1 = lineF1 / sqrt(lineF1(0)*lineF1(0)+lineF1(1)*lineF1(1));

        Vector3d sp_l2; sp_l2 << kl2.startPointX, kl2.startPointY, 1.0;
        Vector3d ep_l2; ep_l2 << kl2.endPointX, kl2.endPointY, 1.0;

        cv::Mat L3dSC1; //线特征的3D起始点在camera1下的坐标
        cv::Mat L3dEC1; //线特征的3D终止点在camera1下的坐标

        LineTriangulate(kl1,kl2,P1,P2,lineF1,lineF2,L3dSC1,L3dEC1);
...
void Tracking::LineTriangulate(const KeyLine &kl1, const KeyLine &kl2, const cv::Mat &P1, const cv::Mat &P2,
                               const Vector3d &klf1, const Vector3d &klf2, cv::Mat &LineStart3D, cv::Mat &LineEnd3D) {
