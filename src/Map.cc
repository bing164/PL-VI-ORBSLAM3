...
void Map::ApplyScaledRotation(const cv::Mat &R, const float s, const bool bScaledVel, const cv::Mat t)
{
    ...
    for (set<MapLine*>::iterator sit=mspMapLines.begin(); sit!=mspMapLines.end(); sit++) {
        ...
        SP = s * Converter::toMatrix3d(Ryw) * pML->GetWorldPos().head(3) + Converter::toVector3d(tyw);
        EP = s * Converter::toMatrix3d(Ryw) * pML->GetWorldPos().tail(3) + Converter::toVector3d(tyw);
