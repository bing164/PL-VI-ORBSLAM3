...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpLinevocabulary(frame.mpLinevocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     n_inliers(frame.n_inliers), n_inliers_pt(frame.n_inliers_pt), n_inliers_ls(frame.n_inliers_ls), mvKeyLineFunctions(frame.mvKeyLineFunctions),
...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor *extractor, Lineextractor *Lineextractor,
             ORBVocabulary *voc, LineVocabulary *voc_l, GeometricCamera *pCamera, cv::Mat &distCoef, const float &bf,
             const float &thDepth, Frame *pPrevF, const IMU::Calib &ImuCalib)
        :mpcpi(NULL),mpORBvocabulary(voc),mpLinevocabulary(voc_l),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
         mpLineextractorLeft(Lineextractor), mpLineextractorRight(static_cast<class Lineextractor*>(NULL)),
         mTimeStamp(timeStamp), mK(static_cast<Pinhole*>(pCamera)->toK()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
         mImuCalib(ImuCalib), mpImuPreintegrated(NULL),mpPrevFrame(pPrevF),mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbImuPreintegrated(false), mpCamera(pCamera),
         mpCamera2(nullptr), mTimeStereoMatch(0), mTimeORB_Ext(0)
{
    ...
    thread threadPoint(&Frame::ExtractORB, this, 0, imGray, 0,0);
    thread threadLine(&Frame::ExtractLine, this, 0, imGray);
    threadPoint.join();
    threadLine.join();
    ...
    mnScaleLevels_l = mpLineextractorLeft->nlevels_l;
    mvScaleFactors_l = mpLineextractorLeft->mvScaleFactor_l;
//    cout << "mvScaleFactors_l = " << mvScaleFactors_l.size() << endl;
    mvInvScaleFactors_l = mpLineextractorLeft->mvInvScaleFactor_l;
    mvLevelSigma2_l =  mpLineextractorLeft->mvLevelSigma2_l;
    mvInvLevelSigma2_l = mpLineextractorLeft->mvInvLevelSigma2_l;
    ...
    N_l = mvKeys_Line.size();
//    cout << "the number of lines = " << N_l << endl;
    mvpMapLines = vector<MapLine*>(N_l, static_cast<MapLine*>(NULL));
    mvbOutlier_Line = vector<bool>(N_l, false);
...
{
    if(flag==0)
        (*mpLineextractorLeft)(im,cv::Mat(),mvKeys_Line,mDescriptors_Line,mvKeyLineFunctions);
    else
        (*mpLineextractorRight)(im,cv::Mat(),mvKeysRight_Line,mDescriptorsRight_Line,mvKeyLineFunctions);
}
...
void Frame::lineDescriptorMAD(vector<vector<DMatch>> matches, double &nn_mad, double &nn12_mad) const {
